...
#                 Created on Oct 2013
#                 Last modified on May 2016 by Hao Yuan
#
###################################################################################################
...
sub indexfastq {
    ...
    if ($infile =~ /rmrep/) {
		...
		while (my $line = readline ($INFILE)) {
			chomp $line;
			if ($line =~ /^@\S+\.f$|^@\S+\.r$/i) { # if we find @ at beginning, and f/r at the end
    ...
    print $INDEX "$id\t$file_pointer\n";
    		...
    		while (my $line = readline ($INFILE)) {
			chomp $line;
			if ($line =~ /^@\S+\s\S+/) { # if we find @ at beginning, and f/r at the end
				if ($id) { # if id exists we first need to write old information
					print $INDEX "$id\t$file_pointer\n";
					$file_pointer = $file_pointerlag;
				}
